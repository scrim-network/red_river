...
path_out_cmip5 <- '/storage/home/jwo118/group_store/red_river/cmip5_wetbias'
...
fpath_log <- file.path('/storage/home/jwo118/group_store/red_river/logs/wetbias.log')
...
results <- foreach(fpath=fpaths_in) %dopar% {
  ...
  for (r in 1:dim(a_wetbias)[1]) {
    ...
    for (c in 1:dim(a_wetbias)[2]) {
      ...
      rc_bias <- mw_bias_correct(merge_rc[,1], merge_rc[,2], idx_train, idx_fut, wetday_bias, win_masks, win_masks1, correct_win=FALSE)
