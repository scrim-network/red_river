'''
Script to compute "spatiotemporal correlograms" for daily temperature.
...
import pandas as pd
import xarray as xr
from esd.downscale.tair import to_anomalies
...
#     downscale_end_year = '1977'
    ...
    # of anomalies and not the absolute temperature values  
    for vname in ds_d.data_vars.keys():
        ds_d[vname] = to_anomalies(ds_d[vname])[0]
#         
...
#                      'Analog Using\nAnomalies (Interp Scale)']
    ...
    for a_mth,a_mthname in zip(months,mth_names):
        ...
        results['Observed'] = validate_spatial_coherence(ds_d.obs, rfunc,months=a_mth).r
    
        df_stcorrel = pd.DataFrame(results)
        df_stcorrel.to_csv(os.path.join(path_out,'stcorrel_tair_anomalies_%s.csv'%(a_mthname,)))